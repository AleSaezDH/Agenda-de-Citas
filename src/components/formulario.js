...
function Formulario ({agregarCitas}) {
    ...
    return <div style={{display:'flex', flexDirection:'column', width:'50%', textAlign:'center', marginLeft:50, marginRight:25}}>
        <Paper elevation={3}>
        <Typography data-testid="titulo" variant="h4" style={{marginTop:15, marginBottom:15}}>Crea tu cita</Typography>
    <form onSubmit={agregarCita} style={{paddingRight:50, paddingLeft:50, paddingBottom:50}}>
        <div style={estilo}>
            <input data-testid="mascota" label="Mascota" name='mascota' onChange={handleChange} value={mascota} fullWidth/>
        </div>

        <div style={estilo}>
            <input data-testid="dueño" label="Dueño" name='propietario' onChange={handleChange} value={propietario} fullWidth/>
        </div>

        <div style={{marginTop:40, marginBottom:40, display:'flex', justifyContent:'space-around'}}>
            <input data-testid="fecha" label="Fecha" type="date" name='fecha' onChange={handleChange} value={fecha} InputLabelProps={{shrink: true}} />
            <input data-testid="hora" label="Hora" type="time" name='hora' onChange={handleChange} value={hora} InputLabelProps={{shrink: true}} />
        </div>

        <input data-testid="sintomas" label="Síntomas" name='sintomas' onChange={handleChange} value={sintomas} multiline rows={4} fullWidth variant='outlined'/>

        <Button data-testid="boton-submit" color='primary' variant='contained'  type='submit' style={{marginTop:40}}>Agregar cita</Button>
    </form>
    </Paper>
